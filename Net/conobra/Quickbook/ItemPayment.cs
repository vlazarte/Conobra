...
namespace Quickbook
{
    public class ItemPayment : ItemAbstract
    {
        ...
        public List<string> fetchAll(ref string err)
        {
            ...
            code = doc["QBXML"]["QBXMLMsgsRs"]["ItemPaymentQueryRs"].Attributes["statusCode"].Value;
            statusMessage = doc["QBXML"]["QBXMLMsgsRs"]["ItemPaymentQueryRs"].Attributes["statusMessage"].Value;

            if (code == "0")
            {
                var data = doc["QBXML"]["QBXMLMsgsRs"]["ItemPaymentQueryRs"];

                var nodeList = data.SelectNodes("ItemPaymentRet");
        ...
        public bool LoadByListID(string lid, ref string err)
        {
            ...
            if (qbook.Connect())
            {
                ...
                code = res["QBXML"]["QBXMLMsgsRs"]["ItemPaymentQueryRs"].Attributes["statusCode"].Value;
                statusMessage = res["QBXML"]["QBXMLMsgsRs"]["ItemPaymentQueryRs"].Attributes["statusMessage"].Value;

                if (code == "0")
                {
                    var node = res["QBXML"]["QBXMLMsgsRs"]["ItemPaymentQueryRs"]["ItemPaymentRet"];
